...
/ PURPOSE      : Execute Web Service DELETE request to unlock a document
/ DESCRIPTION  : This macro executes a web service DELETE request to unlock the supplied document type and document id.
...
/--------------------------------------------------------------------------------------------------------------------
/ EXAMPLE:   %fdx_ws_unlock_doc(object_type=tm_cases,
/                               object_id=202000045,
/                               _ws_out_json=json_out,
/                               _ws_http_code=resp_code);
/--------------------------------------------------------------------------------------------------------------------*/

%macro fdx_ws_unlock_doc(object_type=, object_id=, _ws_out_json=, _ws_http_code=, debug=N);
